                ...
                if(img != null){
                    ...
                    if((faces != null) && (faces.length == 1)){
                        ...
                        if(((method == MANUALLY) && capturePressed) || (method == TIME)){
                            ...
                            if (folder.equals("Test")) {
                                String wholeFolderPath = fh.TEST_PATH + relationship + "$" + name +  "/" + subfolder;
                                new File(wholeFolderPath).mkdirs();
                                fh.saveMatToImage(m, wholeFolderPath + "/");
                            } else {
                                String wholeFolderPath = fh.TRAINING_PATH + relationship + "$" + name;
                                new File(wholeFolderPath).mkdirs();
                                fh.saveMatToImage(m, wholeFolderPath + "/");
                            }
                            ...
                            if(total >= numberOfPictures){
                                Intent intent = new Intent(getApplicationContext(), TrainingActivity.class);  //fixed
