...
public class EditNoteActivity extends AppCompatActivity {
    private EditText inputNote,inputTopicNote;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        inputTopicNote =  findViewById(R.id.input_topicnote);
        dao = NotesDB.getInstance(this).notesDao();
        if (getIntent().getExtras() != null) {
            int id = getIntent().getExtras().getInt(NOTE_EXTRA_Key, 0);
            temp = dao.getNoteById(id);
            inputNote.setText(temp.getNoteText());
            inputTopicNote.setText(temp.getNoteText2());
    ...
    private void onSaveNote() {
        ...
        String topictext = inputTopicNote.getText().toString();

        if (!text.isEmpty()&!topictext.isEmpty()) {
            long date = new Date().getTime(); // get  system time
            // if  exist update els crete new
            if (temp == null) {
                temp = new Note(topictext,text, date);
                dao.insertNote(temp); // create new note and inserted to database
            } else {
                temp.setNoteText(topictext,text);
