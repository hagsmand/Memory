package com.ixidev.simplenotepad.model;

import android.arch.persistence.room.ColumnInfo;
import android.arch.persistence.room.Entity;
import android.arch.persistence.room.Ignore;
import android.arch.persistence.room.PrimaryKey;

/**
 * Created by ixi.Dv on 13/05/2018.
 */

@Entity(tableName = "notes")
public class Note {
    @PrimaryKey(autoGenerate = true)
    private int id; // default value
    @ColumnInfo(name = "topictext")
    public String noteTopicText;
    @ColumnInfo(name = "text")
    public String noteText;
    @ColumnInfo(name = "date")
    private long noteDate;

    @Ignore // we don't want to store this value on database so ignore it
    private boolean checked = false;

    public Note() {
    }

    public Note(String noteTopicText ,String noteText, long noteDate) {
       this.noteTopicText = noteTopicText;
        this.noteText = noteText;
        this.noteDate = noteDate;
    }

    public String getNoteText() {
        return noteText;
    }

   public void setNoteText(String noteTopicText,String noteText) {
        this.noteText = noteText;
        this.noteTopicText = noteTopicText;
    }
    public String getNoteText2() {
        return noteTopicText;
    }

    /*public void setNoteText2(String noteTopicText) {
        this.noteTopicText = noteTopicText;
    }*/

    public long getNoteDate() {
        return noteDate;
    }


    public void setNoteDate(long noteDate) {
        this.noteDate = noteDate;
    }

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public boolean isChecked() {
        return checked;
    }

    public void setChecked(boolean checked) {
        this.checked = checked;
    }

    @Override
    public String toString() {
        return "Note{" +
                "id=" + id +
                ", noteDate=" + noteDate +
                '}';
    }
}
